...
# │   ├── web\_server.py      # FastAPI web server serving static assets and REST API endpoints.

# │   └── sample.ipynb       # Jupyter notebook showing interactive testing.

# ├── tests/

# │   └── test\_pipeline.py   # Pytest suite ensuring the mock and vertex logic is bulletproof.
...
# Inside `.env`, you can toggle `USE\_MOCK=True` (for local development without Vertex API keys) or set it to `False` and supply your actual `PROJECT\_ID` and `LOCATION` to use the real Google Vertex AI models.
...
# py -m uv run python -m src.web\_server

# ```

# \*(Or simply `python -m src.web\_server` if your virtual environment is already activated).\*
...
# \- Move the top slider to adjust the number of retrieved matches (`top\_k`) interactively.
